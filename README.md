...
A modern, fully responsive developer portfolio built with Vue 3, Tailwind CSS v4, and Vite. Content (skills, projects, experience, etc.) is loaded live from Firebase Realtime Database. Features dark/light theme switching, smooth scroll-reveal animations, a typewriter effect, and a working contact form powered by EmailJS.
...
- **Firebase-Backed Content** — all portfolio data (skills, projects, experience, achievements) is fetched from Firebase Realtime Database at runtime
...
│   ├── firebase.js             # Firebase app init & Realtime Database connection
...
│   │   ├── LoadingSpinner.vue  # Loading state while Firebase data fetches
│   │   └── FooterSection.vue   # Links, socials, copyright
│   ├── composables/
│   │   ├── usePortfolioData.js # Fetches portfolio content from Firebase Realtime Database
...
│   └── data/
│       └── portfolio.js        # Fallback/seed portfolio content
├── tests/
...
All variables must be prefixed with `VITE_` to be exposed to the Vite client bundle. Get the Firebase values from Firebase Console → Project Settings → General → Your apps → SDK config.
...
Portfolio content (personal info, skills, projects, experience, achievements, stats) is loaded at runtime from **Firebase Realtime Database** via `src/composables/usePortfolioData.js`. To update the site's content:

1. Open your Firebase project's Realtime Database in the Firebase Console
2. Edit the relevant nodes (see `src/data/portfolio.js` for the expected shape/fallback structure)
3. Changes appear on the site on next page load — no rebuild or redeploy required

`src/data/portfolio.js` still defines the fallback/seed data shape used if Firebase is unreachable, and is a useful reference for the data structure.
